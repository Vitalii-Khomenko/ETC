...
function renderMachineRanges(options = {}) {
    ...
    const visibleMachines = diagramData.machines.filter(group => group.sections.some(section => section.equipment.length > 0));
    ...
    summaryEl.textContent = `Machines: ${diagramData.totals.machines} | Sections shown: ${sectionCount} | Matches to replace: ${diagramData.totals.candidates}`;

    if (visibleMachines.length === 0) {
        const empty = document.createElement("div");
        empty.className = "empty-machine";
        empty.textContent = "No machine sections were found for the current filters.";
        body.appendChild(empty);
        return;
    }
